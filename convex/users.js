...
export const getCurrentUser = query({
  handler: async (ctx) => {
    return await ctx.runQuery(internal.users.getUserInternal);
  },
});

// Internal version of getCurrentUser for use in mutations
...
export const completeOnboarding = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!user) {
      throw new Error("User not found. You must be logged in to complete onboarding.");
    }
...
// Development tool: Upgrade current user to Pro
export const upgradeSelfToPro = mutation({
  args: {},
  handler: async (ctx) => {
    const user = await ctx.runQuery(internal.users.getUserInternal);

    if (!user) {
      throw new Error("User not found. You must be logged in.");
    }
    ...
    return { success: true, message: "You are now a Pro! 🚀" };
  },
});
