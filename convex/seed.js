...
export const run = internalMutation({
  handler: async (ctx) => {
    // 1. Create Organizer
    let organizer = await ctx.db.query("users").filter(q => q.eq(q.field("email"), "organizer@eventhub.com")).first();

    if (!organizer) {
      const organizerId = await ctx.db.insert("users", {
        email: "organizer@eventhub.com",
        tokenIdentifier: "seed-organizer-token",
        name: "EventHub Team",
    ...
    // 2. Create Dummy Users (Attendees)
    const userIds = [];
    for (const userData of SAMPLE_USERS) {
      let user = await ctx.db.query("users").filter(q => q.eq(q.field("email"), userData.email)).first();
    ...
    // 3. Create Events and Registrations
    for (const eventData of SAMPLE_EVENTS) {
      const startDate = getRandomFutureDate();
      const endDate = getEventEndTime(startDate);

      // Determine random number of registrations (up to capacity or 10, whichever is lower/safe)
      const maxRegs = Math.min(eventData.capacity, userIds.length, 8);
      const registrationCount = Math.floor(Math.random() * (maxRegs + 1));
      ...
      const event = {
        ...
        country: "India",
        registrationCount: 0, // Initial count, will update after registering
        createdAt: Date.now(),
        updatedAt: Date.now(),
      };
      ...
      const selectedNetUsers = shuffledUsers.slice(0, registrationCount);

      for (const userId of selectedNetUsers) {
        ...
        await ctx.db.insert("registrations", {
          ...
          checkedIn: false,
          status: "confirmed",
          registeredAt: Date.now() - Math.floor(Math.random() * 86400000), // Random time in last 24h
        });
      }

      // Update event with actual registration count
      await ctx.db.patch(eventId, { registrationCount });
    }

    console.log(`✅ Successfully seeded ${createdEvents.length} events and registrations!`);
    return {
      success: true,
